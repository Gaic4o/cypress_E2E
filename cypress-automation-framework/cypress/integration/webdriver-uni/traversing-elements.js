...
describe("Traversing DOM elements in Cypress", () => {
    ...
    beforeEach(() => {
      ...
      it("children() to get the children of DOM elements", () => {
        cy.get('.traversal-breadcrumb').children('.active').should('contain', 'Contact Us');  // ol 안에 li 태그들 children 속성으로 가져 옵니다.
      });
      ...
      it("eq() to retrieve a specific element based on index", () => {
          cy.get('.traversal-drinks-list > *').eq(2).should('contain', 'Milk') // select 5개 요소를 찾은 다음 -> 2번쨰 있는 값을 선택 합니다.
      });
      ...
      it("find() to retrieve DOM elements of a given selector", () => {
        // find 으로 모든 링크를 찾은 뒤, 첫 번째 일은 마우스 오른쪽 버튼을 클릭 후 태그 보기.
